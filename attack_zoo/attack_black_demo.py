...
class attack_black_demo():
    ...
    def attack(self, model, notes, formatter):
        ...
        for dataset_idx, data in enumerate(notes):
            #print("INSTANCE %d---------------------------------------------------------------------" %dataset_idx)
            data = formatter.process([data], self.config, "test")
            ...
            while True:
                ...
                old_logits, prediction = result["logits"], result["prediction"].long()
                #if iter_num == 1:
